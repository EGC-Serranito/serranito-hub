...
def mock_user_profile_repo():
    ...
    with patch.object(
        UserProfileRepository, "update", return_value="user_updated"
    ) as mock_update:
        yield mock_update
...
def test_update_profile_success(
    user_profile_service, mock_user_profile_repo, mock_form
):
    """Test exitoso de update_profile cuando la validación es correcta."""
    user_profile_id = 1
    updated_instance, errors = user_profile_service.update_profile(
        user_profile_id, mock_form
    )
