...
class BotIntegrationRepository(BaseRepository):
    ...
    def get_tree_nodes_by_user(self, user_id):
        ...
        """
        try:
            # Consultar todos los nodos de árbol (TreeNode) asociados al usuario
            nodes = db.session.query(TreeNode).filter(TreeNode.user_id == user_id).all()
            return nodes
    ...
    def create_node_route_add_chat(self, user_id, name, parent_id, path, single_child):
        try:
            ...
            path_child = path_child + "/NOTIFICATION PREFERENCE"
            ...
            path_child = path_child + "/TYPES OF NOTIFICATION"
            types_node = TreeNode(
                user_id=user_id,
            ...
            path_child = path_child + "/NEW MESSAGES"
            ...
            path_child = path_child + "/IMMEDIATELY"
            ...
            path_child = path_child + "/SYSTEM ERRORS"
            ...
            path_child = path_child + "/FEATURES"
    ...
    def create_node_route_add_types_notification(self, user_id, name, parent_id, path, single_child):
        try:
            db.session.query(TreeNode).filter(TreeNode.parent_id == parent_id).delete()
            db.session.commit()
            # Crear el nodo principal
            new_node = TreeNode(
                user_id=user_id,
